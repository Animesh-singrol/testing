...
const { registerPatient, getAllPatients, getPatientsReport, createPatientRecords } = require('../controllers/patientController');
const imageUploadMiddleware = require('../middlewares/imageMIddleware');
const { body, validationResult } = require('express-validator');

const router = express.Router();

router.post('/fetch', getAllPatients);
...
const validatePatientRegistration = [
  ...
  body('doctorId')
    ...
    .isInt({ min: 1 }).withMessage('Doctor ID must be a valid number'),

  body('medicalHistory')
    .optional()
    .isLength({ max: 500 }).withMessage('Medical history should not exceed 500 characters'),
];
...
//sync patients
router.post('/sync', createPatientRecords);
